...
> **⚠ "Nos Bilans" is HIDDEN since 26/08/2026** — `BILANS_ENABLED = false` in `src/constants/features.ts`, at Dr Aziz's request while the lab reworks the bundles ("juste le cacher pour le moment"). Nothing was deleted: the tab definition, `sortedBilans`, `BilanCard`, the cart logic and the PDF quote are all untouched. **Flip the flag back to `true` and everything returns**, here and in the four other places listed in the flag's own comment. With one tab left, `TabsNavigation` is not rendered at all (`tabs.length > 1`) — a lone tab reads as a filter you cannot change. `?tab=bilans` links keep working: `TAB_ORDER` no longer contains `bilans`, so they silently land on the catalogue instead of an empty tab.
...
- **Never hardcode the header height again.** The sticky search/tabs container and the fixed desktop cart panel both sit *below* the site header. They used the literal `64px` (`sticky top-[64px]`, `top: '64px'`, `height: 'calc(100vh - 64px)'`); they now read **`var(--header-total-height)`**, declared in `src/styles/components/navigation.css`. That variable is `64px + env(safe-area-inset-top)`: in an installed PWA the header grows by the status-bar inset, so any surviving `64px` literal would leave a gap (or slide under the header) on iPad/iPhone. The bare `var(--header-height)` (64px, no inset) exists too — use it only for things *inside* the header.
