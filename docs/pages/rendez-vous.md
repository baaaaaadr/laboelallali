...
- **Date & hour are bound to the lab's real opening hours** — identical wiring to `/glabo`, described in full in `docs/pages/glabo.md` §1. In short: `const now = useNow()` (null until mount), `generateTimeSlots(selectedDate, now)`, `filterDate={(date) => hasBookableSlots(date, now)}`, `selectedDate` seeded in a `useNow` effect via `nextBookableDate`, and `handleDateChange` clearing a `selectedTime` the new date no longer offers. Lun-Ven 07:30→18:15, Sam 07:30→12:45, dimanche fermé, and past hours of the current day are dropped.
