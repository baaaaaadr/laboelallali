...
- Up to 3 `.card` tiles from `computeResultsStats(results)`: **Bilans** (count, always), **Dernier bilan** (localized long date of `newestIso`, `—` if none), **Suivi depuis** (oldest year, shown ONLY when `oldestYear < newestYear` — multi-year history). Labels `resultats.stats_{bilans,last,since}_label` (fr + ar).
- **No plural keys** — numbers/dates go through `Intl` (`ar-MA`/`fr-FR`) so Arabic renders Arabic-Indic digits (matches the year headers). `tabular-nums` on values. Reuses `.card` + the icon-in-`rounded-lg bg-[var(--color-bordeaux-primary)]/10` motif; static grid-cols classes for the Tailwind scanner; RTL via grid/flex + `text-start`.
