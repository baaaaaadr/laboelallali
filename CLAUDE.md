...
- `sendAppointmentRequestEmail` (`onDocumentCreated` on `appointmentRequests/{docId}`) — SendGrid email + optional Storage attachment; stamps status. Params via `defineString("sendgrid.key"/"sendgrid.sender"/"lab.email")` (dotted names — they do NOT match the underscore names in `functions/.env.example`).
- `cleanupExpiredPrescriptions` (`onSchedule` daily 02:00 Europe/Paris) — deletes prescription files past 30-day `expiresAt`.
- `fetchResults` (`onCall`) — the CyberLab bridge (from `functions/src/cyberlab/fetchResults.ts`).

**CyberLab bridge is viewer-only:** results are fetched on demand and returned verbatim to the client — never written to Firestore, never logged, `Cache-Control: no-store`. Flow: reject unauthenticated; take `uid` from auth; read `users/{uid}` and take `requester_id` + `type ∈ {patient,medecin,correspondant}` **exclusively from the profile (never from client input — anti-spoof)**; call `callCyberlab(cfg, {type, requester_id, max_results:50})`. Every failure maps to a generic French `HttpsError`; only `kind`/`status` is logged, never the response body (may contain medical data).
