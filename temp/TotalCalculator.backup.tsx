...
export default function TotalCalculator({
  ...
  return (
    <div 
      ref={containerRef}
      className="floating-total-calculator dark:floating-total-calculator-dark"
    ...
    >
      ...
      <div className="text-lg font-bold">
        <span className="text-white">{translationsUI.total}:</span>
        <span className="text-[var(--bordeaux-pale)] dark:text-[var(--bordeaux-light)]"> {totalCost.toLocaleString()} {currencyLabel}</span>
      </div>
      
      {/* Send via WhatsApp button */}
      <button 
        onClick={handleSendViaWhatsApp}
        className="mt-3 py-2 px-4 rounded-lg border border-white/50 hover:border-white/80 bg-white/5 hover:bg-white/10 text-white text-sm font-medium flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-white/50 focus:ring-offset-2 focus:ring-offset-[var(--primary-bordeaux)]"
      ...
      <style jsx global>{`
        ...
        /* Dark mode styles */
        .dark .floating-total-calculator {
          background-color: var(--bordeaux-darker) !important;
          box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.25), 0 4px 6px -2px rgba(0, 0, 0, 0.15) !important;
        }
        
        .dark .floating-total-calculator button:hover {
